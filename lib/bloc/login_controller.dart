...
class LoginController extends GetxController {
  ...
  bool dataAvailable(String key) {
    return dataLength.value > 0 && contentDataMap.containsKey(key)
        ? true
        : false;
  }
  ...
  void submitLogin(String _uname, String _password, String roles) async {
    ...
    try {
      ...
      http.Response response =
          await http.post(url, body: json, headers: requestHeaders);
      ...
      if (response.statusCode == 200) {
        ...
        if (check.value.responseCode == "00") {
          CheckUser checkPassword2 =
              CheckUser.fromJson(jsonDecode(response.body));
          check.value.userData = checkPassword2.userData;
          login.value = RxBool(true);

          print(checkPassword2.userData.name);
      ...
      check.value = CheckPassword(
          responseCode: '05', responseMessage: 'error', userData: UserModel());
      login.value = RxBool(false);
      loginState.value = 5;
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        imageData.value = ImageData(
            name: "NA",
            content: response.statusCode.toString() + " : " + response.body);
      }
    } catch (e) {
  ...
  void submitChangePassword(String _pass, String _newPass) async {
    ...
    ChangePassword changePassword = ChangePassword(
        userId: check.value.userId,
        token: check.value.token,
        oldPass: passDigest.toString(),
        newPass: newPassDigest.toString());
    ...
    try {
      ...
      print(
          "submitChangePassword response status : ${response.statusCode.toString()} body ${response.body}");
